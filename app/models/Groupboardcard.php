...
class Groupboardcard extends \Phalcon\Mvc\Model
{
    ...
    {
        $this->setSchema("taff");
        $this->setSource("groupboardcard");
        $this->belongsTo('cardListId', '\Groupboardlist', 'listId', ['alias' => 'Groupboardlist']);
     ...
     * @return Groupboardcard[]|Groupboardcard|\Phalcon\Mvc\Model\ResultSetInterface
     */
     ...
     *
     * @param mixed $parameters
     * @return Groupboardcard|\Phalcon\Mvc\Model\ResultInterface
     */
    ...
    {
        $card = Groupboardcard::find();
        return count($card);
    }

    public function insertGroupBoardCard($listId,$owner,$title,$description,$archive,$status)
    {
        $card = new Groupboardcard();
        $index = $card->countCard();
        $id = "GC".str_pad($index,3,'0',STR_PAD_LEFT);
